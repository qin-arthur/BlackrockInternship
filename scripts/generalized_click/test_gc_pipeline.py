    ...
    with NWBHDF5IO(str(nwb_path), 'r') as io:
        ...
        for idx, epoch in epochs_df.iterrows():
            ...
            try: 
                ...
                print("Incompatible trial, extraction error")
                continue
...
def extract_trial_data(nwbfile, epoch: pd.Series, verbose: bool = False) -> Optional[Dict]:
    ...
    if 'behavior' in nwbfile.processing:
        ...
        # Look for ClickState timeseries
        for ts_name, ts_obj in behavior_module.containers.items():
            if 'ClickState' in ts_name and task in ts_name:
                click_data = extract_timeseries_data(ts_obj, start_time, duration)
                break
    ...
    if click_data is None:
        if verbose:
            print(f"      No click data found for {task} epoch")
