...
import pandas as pd
filepath = 'C:/Users/aqin/Downloads/generalized-click/generalized-click/'

import numpy as np

def preprocess_generalized_click(nwbfile, bin_ms=20, epochs=('Reach','Center')):
    """
    From a loaded Generalized‐Click NWB file, extract:
      • features: (n_segs, n_time_bins, n_neurons)
      • behavior: (n_segs, n_time_bins, 4)   ← [x_pos, y_pos, x_vel, y_vel]
      • conditions: (n_segs,)                ← 0–7 for Reach directions, 8–15 for Center
    
    Parameters
    ----------
    nwbfile
        An object giving you per‐trial spike times, kinematics, and target positions.
    bin_ms : int
        Bin width in milliseconds (default 20 ms → 50 Hz).
    epochs : tuple of str
        Names of the epochs to extract from each trial (default “Reach” and “Center”).
    
    Returns
    -------
    features, behavior, conditions
        Ready to pass into MINT.fit(), which expects
        features.shape  == (n_trials, n_time_bins, n_neurons)
        behavior.shape  == (n_trials, n_time_bins, n_behavioral_features)
        conditions.shape== (n_trials,) :contentReference[oaicite:3]{index=3}
    """
    seg_feats   = []
    seg_behav   = []
    seg_conds   = []

    # Precompute bin edges (we’ll assume all epochs share same duration)
    for trial in nwbfile.trials:
        # retrieve target position once per trial
        tx, ty = trial.target_pos  # e.g. trial.targets['position']

        # figure out which of 8 reach‐directions this is
        angle = np.arctan2(ty, tx)
        dir_bin = int(np.round((angle / (2*np.pi)) * 8)) % 8

        for ei, ep_name in enumerate(epochs):
            ep = trial.epochs[ep_name]
            t0, t1 = ep.start_time, ep.end_time
            # how many bins?
            n_bins = int(np.ceil((t1 - t0) * 1000.0 / bin_ms))
            edges  = t0 + np.arange(n_bins+1) * (bin_ms/1000.0)

            # 1) BIN SPIKES → (n_bins, n_neurons)
            #    assume ep.spike_times is list-of-arrays: one array per neuron
            spikes = ep.spike_times  
            binned = np.zeros((n_bins, len(spikes)), dtype=int)
            for u, times in enumerate(spikes):
                counts, _ = np.histogram(times, bins=edges)
                binned[:,u] = counts

            # 2) EXTRACT & BIN KINEMATICS → (n_bins,4)
            #    assume ep.position is an (T,2) array sampled at high rate
            pos = ep.position
            # downsample by averaging into same edges
            # here we’ll linearly interpolate position at bin centers
            centers = edges[:-1] + 0.5*(bin_ms/1000.0)
            x = np.interp(centers, ep.time, pos[:,0])
            y = np.interp(centers, ep.time, pos[:,1])
            # velocities via first‐difference
            vx = np.diff(x, prepend=x[0]) / (bin_ms/1000.0)
            vy = np.diff(y, prepend=y[0]) / (bin_ms/1000.0)
            beh = np.stack([x,y,vx,vy], axis=1)

            # 3) CONDITION INDEX
            #    0–7 for Reach, 8–15 for Center
            cond = dir_bin + (0 if ep_name=='Reach' else len(epochs[0]))

            seg_feats .append(binned)
            seg_behav .append(beh)
            seg_conds .append(cond)

    features   = np.stack(seg_feats, axis=0)      # (n_segs, n_bins, n_neurons)
    behavior   = np.stack(seg_behav, axis=0)      # (n_segs, n_bins, 4)
    conditions = np.array(seg_conds, dtype=int)   # (n_segs,)

    return features, behavior, conditions
...
def process_nwb_directory(nwb_dir: Union[str, Path], 
    ...
    if merge_sessions and len(final_sessions) > 1:
        ...
        final_sessions["merged"] = merged_sessions["merged"]
...
def process_single_nwb(nwb_path: Union[str, Path], verbose: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[Dict]]:
    ...
    with NWBHDF5IO(str(nwb_path), 'r') as io:
        ...
        trial_data = []
        df_filtered = epochs_df[epochs_df["epoch_type"].eq("Reach")]
        for idx, epoch in df_filtered.iterrows():
            trial_info = extract_trial_data(nwbfile, epoch, verbose=verbose)
            if trial_info['target'] is not None:
        ...
        if verbose:
            print(f"    Extracted {len(trial_data)} MINT-compatible trials")
...
def extract_trial_data(nwbfile, epoch: pd.Series, verbose: bool = False) -> Optional[Dict]:
    ...
    if 'ecephys' in nwbfile.processing:
        ...
        for ts_name, ts_obj in ecephys_module.containers.items():
            if 'BinnedSpikes' in ts_name and task in ts_name:
                ...
                spike_data = extract_timeseries_data(ts_obj, start_time, stop_time)
                break
    ...
    if 'behavior' in nwbfile.processing:
        ...
        # Look for MousePosition timeseries
        for ts_name, ts_obj in behavior_module.containers.items():
            if 'MousePosition' in ts_name and task in ts_name:
                position_data = extract_timeseries_data(ts_obj, start_time, stop_time)
                break
        
        # Look for MouseVelocity timeseries
        for ts_name, ts_obj in behavior_module.containers.items():
            if 'MouseVelocity' in ts_name and task in ts_name:
                velocity_data = extract_timeseries_data(ts_obj, start_time, stop_time)
                break
        
        # Look for TargetPosition timeseries
        for ts_name, ts_obj in behavior_module.containers.items():
            if 'TargetPosition' in ts_name and task in ts_name:
                target_data = extract_timeseries_data(ts_obj, start_time, stop_time)
                break
    ...
    if target_data.shape[0] != 0:
        target_pos = target_data[0]
    
    return {
        'spikes': spike_data,  # (T, n_neurons)
...
def extract_timeseries_data(timeseries_obj, start_time: float, stop_time: float) -> np.ndarray:
...
def format_trial_arrays(trial_data: List[Dict], verbose: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    n_neurons = trial_data[0]['spikes'].shape[1]
    max_T = max(trial['spikes'].shape[0] for trial in trial_data)
    # missing = 0
    # for feature in trial_data: 
    #     if feature == None:
    #         missing += 1
    # n_bfeatures = len(trial_data) - 4 - missing
    
    if verbose:
        print(f"    Formatting arrays: {n_trials} trials, {max_T} time bins, {n_neurons} neurons")
    
    # Initialize arrays
    spikes = np.zeros((n_trials, max_T, n_neurons))
    # behavior = np.zeros((n_trials, max_T, n_bfeatures))
    behavior = np.zeros((n_trials, max_T, 4))
    
    # Fill arrays with padding
    for i, trial in enumerate(trial_data):
        trial_T = trial['spikes'].shape[0]
        ...
        # min_len = min(pos.shape[0], vel.shape[0], click.shape[0], trial_T)
        min_len = min(pos.shape[0], vel.shape[0], trial_T)
        
        behavior[i, :min_len, 0] = pos[:min_len, 0]  # x position
        behavior[i, :min_len, 1] = pos[:min_len, 1]  # y position
        behavior[i, :min_len, 2] = vel[:min_len, 0]  # x velocity
        behavior[i, :min_len, 3] = vel[:min_len, 1]  # y velocity
        # if (click is not None):
        #     behavior[i, :min_len, 4] = click[:min_len,]
        # if (target is not None):
        #     behavior[i, :2, 5] = target
...
def generate_condition_ids(trial_data: List[Dict], verbose: bool = True) -> np.ndarray:
    ...
    for trial in trial_data:
        ...
        condition_key = (task, epoch_type, tuple(target))
...
def apply_global_neuron_filtering(sessions: Dict[str, Tuple], 
    ...
    if verbose:
        ...
        print(f"  Data shape: {spikes.shape[0]} trials, {spikes.shape[1]} time bins, {spikes.shape[2]} neurons")
    ...
    mean_rates_per_neuron = np.mean(spikes, axis=(0, 1)) / bin_size_sec
...
def merge_session_data_unified(sessions: Dict[str, Tuple], 
    ...
    for key in session_data.keys():
        session_data[key] = session_data[key][:-1]
        
    
    # Check dimensions
    first_session = list(session_data.values())[0]
    n_neurons = first_session[0].shape[2]
    max_T = max(data[0].shape[1] for data in session_data.values())
    ...
    for session_name, session_tuple in session_data.items():
        ...
        assert spikes.shape[2] == n_neurons, f"Session {session_name} has {spikes.shape[2]} neurons, expected {n_neurons}"
        
        # Pad to max_T if needed
        if spikes.shape[1] < max_T:
            padded_spikes = np.zeros((spikes.shape[0], max_T, spikes.shape[2]))
            padded_behavior = np.zeros((behavior.shape[0], max_T, behavior.shape[2]))
            
            padded_spikes[:, :spikes.shape[1], :] = spikes
            padded_behavior[:, :behavior.shape[1], :] = behavior
